...
async function handleImageFromPage(request, sender, sendResponse) {
  try {
    ...
    const endpoints = [
      'https://accessable-fyp.onrender.com/analyze-image',
      'https://accessable-fyp.onrender.com/analyze_image/',
      'https://accessable-fyp.onrender.com/analyze_image'
    ];
    ...
    if (!response) {
      throw new Error('All API endpoints failed. Please check backend connection.');
    }
    
    console.log('API Response Status:', response.status, 'from', successfulEndpoint);
